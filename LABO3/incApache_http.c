...
/*** TO BE DONE 4.0 START ***/
    ...
    struct stat tmp;
	 ...
	 if (stat_p == NULL) stat_p = &stat_buffer;
    if (stat(HTML_404, stat_p) < 0) fail_errno("Errore nella stat 404");
    file_size = stat_p->st_size;
    file_modification_time = stat_p->st_mtime;
...
/*** TO BE DONE 4.0 START ***/
	 ...
	 if (stat_p == NULL) stat_p = &stat_buffer;
    if (stat(HTML_501, stat_p) < 0) fail_errno("Errore nella stat 501");
    file_size = stat_p->st_size;
    file_modification_time = stat_p->st_mtime;
...
/*** TO BE DONE 4.0 START ***/
		...
		if (sendfile(client_fd, fd, NULL, file_size) < 0) fail_errno("Errore nella sendfile");
...
/*** TO BE DONE 4.0 START ***/
                ...
                } else http_method -= METHOD_CONDITIONAL;
